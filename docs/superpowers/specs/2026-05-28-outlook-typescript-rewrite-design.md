...
|---|---|---|
| **`core`** (`@alavida-ai/outlook-core`) | MSAL device-code flow; token cache at `~/.outlook-cli/tokens.json` (0600); a configured `@microsoft/microsoft-graph-client` instance wired to MSAL via the first-party `AuthProvider`; thin typed wrappers around `client.api(...)` for our specific endpoints; pagination via `PageIterator`; error mapping that lifts SDK errors (`GraphError`) into our taxonomy (`AuthError`, `NotFoundError`, `ThrottledError`, `NetworkError`). Pure library — no stdout/stderr, no `process.exit`. | `OutlookClient` with `.mail`, `.calendar`, `.contacts`, `.me`. Free functions: `loginDeviceCode()`, `logout()`, `status()`, `getAccessToken()`. |
...
| Graph HTTP | `@microsoft/microsoft-graph-client` (classic SDK) + `@microsoft/microsoft-graph-types` (type-only) | The classic SDK has **one transitive dep (`tslib`)** and zero install scripts, so it doesn't materially expand our supply-chain surface. What it does give us is pagination via `PageIterator`, built-in `Retry-After` / 429 throttling middleware, and a first-party MSAL `AuthProvider` adapter. All of those are correctness-critical and easy to get wrong hand-rolled. We deliberately avoid the newer Kiota-based `@microsoft/msgraph-sdk-javascript` — it's where MS is steering long-term but it's still maturing and pulls in heavier transitive deps. The classic SDK is what everyone actually ships against today. |
